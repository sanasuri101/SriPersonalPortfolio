  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? "py-4 glassmorphism backdrop-blur-lg"
          : "py-6 bg-transparent"
      )}
    >
      ...
      {/* Mobile Navigation */}
      {mobileMenuOpen && (
        <nav className="md:hidden glassmorphism mt-4 p-4 rounded-lg animate-fade-in">
          <div className="flex flex-col space-y-2">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="px-4 py-3 text-sm font-medium hover:bg-muted/20 rounded-md transition-colors"
